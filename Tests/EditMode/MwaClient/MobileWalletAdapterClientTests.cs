...
{
    /// <summary>
    /// EditMode tests for MobileWalletAdapterClient JSON-RPC request building.
    /// Uses MockMessageSender to capture wire payloads without a real WebSocket.
    ...
    public class MobileWalletAdapterClientTests
    {
        ...
        /// <summary>
        /// Decodes the last captured message from MockMessageSender into a JsonRequest.
        /// MobileWalletAdapterClient sends encrypted payloads only after ECDH — but
        /// PrepareAuthRequest is exercised via Authorize() which calls SendRequest(),
        /// which in turn calls _messageSender.Send(). The raw JSON is embedded in the
        /// encrypted frame; we extract it by deserialising the outer byte[] as UTF-8
        /// JSON directly (valid because no session key is established yet, so the
        /// payload is sent as plain JSON before encryption is set up — matching the
        /// actual MWA handshake flow where HELLO_REQ precedes Authorize).
        ///
        /// NOTE: If the implementation changes to encrypt before ECDH, update this
        /// helper to call GenerateSessionEcdhSecret first.
        /// </summary>
        ...
        // -------------------------------------------------------------------------
        // Test 6 — Authorize builds correct JSON-RPC request
        // -------------------------------------------------------------------------
        ...
        // -------------------------------------------------------------------------
        // Test 7 — Authorize rejects relative identityUri
        // -------------------------------------------------------------------------

        [Test]
        public void Authorize_ThrowsArgumentException_WhenIdentityUri_IsRelative()
        {
            // Arrange — relative URI violates the MWA spec
        ...
        {
            // Arrange — null identityUri is explicitly permitted
        ...
        {
            // Arrange — iconUri must be relative per PrepareAuthRequest validation
